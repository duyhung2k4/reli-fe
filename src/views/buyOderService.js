...
class BuyOrderService {
  async addItemToBuyOder(data) {
    return await axios({
      method: "post",
      url: "http://localhost:5000/api/BuyOrder",
      headers: {
        accepts: "*/*",
  ...
  async getByOrderById(userId) {
    const res = await axios.get(
      `http://localhost:5000/api/BuyOrder/user/${userId}`
    );
    return res;
  }
  async getByOrderBySellerName(sellerName) {
    const res = await axios.get(
      `http://localhost:5000/api/BuyOrder/seller/${sellerName}`
    );
    return res;
  }
  async updateStatus(id) {
    try {
      const response = await axios.put(
        `http://localhost:5000/api/BuyOrder/${id}/update-status`
      );
